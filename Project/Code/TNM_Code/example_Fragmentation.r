...
patch_Paths = list.dirs(path = gpath, recursive = TRUE) %>% str_subset(pattern = "landscape_") %>% str_subset(pattern = "txt/Results") %>% mixedsort()
...
read_Output = function(res_Path, res) {
    ...
    run = str_split(res_Path, pattern = "/")[[1]][12] %>% str_split(., pattern = "_") %>% simplify() %>% nth(2)
    patch = str_split(res_Path, pattern = "/")[[1]][13] %>% str_split(., pattern = "_") %>% simplify() %>% nth(2)
...
ggplot(totalPops_SE, aes(x = g, y = mean_n, fill = patch)) + geom_line() + geom_ribbon(aes(ymax = mean_n + SE, ymin = mean_n - SE), alpha = 0.2) + 
    labs(x = "Generation", y = "Mean Total Population") + theme_bw()
...
cellRich_XY = cellRich %>% left_join(cell_XY, by = c("c" = "cell")) %>% group_by(g, c, patch, x, y) %>% summarise(median_rich = median(n))
