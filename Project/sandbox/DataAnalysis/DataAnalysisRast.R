...
gpath = "/home/frcovell/Project/sandbox/Testing intraspecific competition/Intra0.1"
setwd(gpath)

#### Read in paths for results for all seeds and runs
raster_Paths = list.dirs(path = gpath, recursive = TRUE) %>% str_subset(pattern = "RasterLandscapePID") %>% str_subset(pattern = "txt/Results") %>% mixedsort()
...
ggplot(totalPops_SE, aes(x = g, y = mean_n, fill = patch)) + 
  ...
  labs(title ="Raster intraspecific competition 0.1", x = "Generation", y = "Mean Total Population") + 
  theme_bw()
...
ggplot(richness_SE, aes(x = g, y = mean_Rich, fill = patch)) + geom_line() + 
  ...
  labs(title ="Raster intraspecific competition 0.1", x = "Generation", y = "Mean Species Richness") + 
  theme_bw()
