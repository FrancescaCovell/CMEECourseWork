...
gpath = "/home/frcovell/Project/sandbox/Supplementary/landGen"
setwd(gpath)
...
#Visualise
AmatotalPops_SE = AmatotalPops %>% group_by(g, patch) %>% summarise(mean_n = mean(n), SE = SE(n))
AmatotalPopsPlot<-ggplot(AmatotalPops_SE, aes(x = g, y = mean_n, fill = patch)) + 
  geom_line() +
  geom_ribbon(aes(ymax = mean_n + SE, ymin = mean_n - SE), alpha = 0.2) + 
  labs(title ="Amalagamation Landscape Cover", x = "Generation", y = "Mean Total Population") + 
  theme_bw()
ggexport(AmatotalPopsPlot, filename = "AmatotalPops.pdf")
...
Amarichness<-ggplot(Amarichness_SE, aes(x = g, y = mean_Rich, fill = patch)) + geom_line() + 
  ...
  labs(title ="Amalagamation Landscape Cover", x = "Generation", y = "Mean Species Richness") + 
  theme_bw()
ggexport(Amarichness, filename = "Amarichness.pdf")

RandtotalPops_SE = RandtotalPops %>% group_by(g, patch) %>% summarise(mean_n = mean(n), SE = SE(n)) 
RandtotalPops_SE = RandtotalPops %>% group_by(g, patch) %>% summarise(mean_n = mean(n), SE = SE(n))
RandtotalPopsPlot<-ggplot(RandtotalPops_SE, aes(x = g, y = mean_n, fill = patch)) + 
  geom_line() +
  geom_ribbon(aes(ymax = mean_n + SE, ymin = mean_n - SE), alpha = 0.2) + 
  labs(title ="Random Landscape Cover", x = "Generation", y = "Mean Total Population") + 
  theme_bw()
ggexport(RandtotalPopsPlot, filename = "RandtotalPops.pdf")

Randrichness_SE = RandtotalPopSpecs %>% filter(n > 0) %>% group_by(g, seed, run, patch) %>% summarise(rich = length(n)) %>% ungroup() %>% 
  group_by(patch, g) %>% summarise(mean_Rich = mean(rich), SE = SE(rich))

Randrichness<-ggplot(Randrichness_SE, aes(x = g, y = mean_Rich, fill = patch)) + geom_line() + 
  geom_ribbon(aes(ymax = mean_Rich + SE, ymin = mean_Rich - SE), alpha = 0.2) +
  labs(title ="Random Landscape Cover", x = "Generation", y = "Mean Species Richness") + 
  theme_bw()
ggexport(Randrichness, filename = "Randrichness.pdf")

RasttotalPops_SE = RasttotalPops %>% group_by(g, patch) %>% summarise(mean_n = mean(n), SE = SE(n)) 
RasttotalPops<-ggplot(RasttotalPops_SE, aes(x = g, y = mean_n, fill = patch)) + 
  geom_line() +
  geom_ribbon(aes(ymax = mean_n + SE, ymin = mean_n - SE), alpha = 0.2) + 
  labs(title ="Raster Landscape Cover", x = "Generation", y = "Mean Total Population") + 
  theme_bw()
ggexport(RasttotalPops, filename = "RasttotalPops.pdf")

Rastrichness_SE = RasttotalPopSpecs %>% filter(n > 0) %>% group_by(g, seed, run, patch) %>% summarise(rich = length(n)) %>% ungroup() %>% 
  group_by(patch, g) %>% summarise(mean_Rich = mean(rich), SE = SE(rich))

Rastrichness<-ggplot(Rastrichness_SE, aes(x = g, y = mean_Rich, fill = patch)) + geom_line() + 
  geom_ribbon(aes(ymax = mean_Rich + SE, ymin = mean_Rich - SE), alpha = 0.2) +
  labs(title ="Raster Landscape Cover", x = "Generation", y = "Mean Species Richness") + 
  theme_bw()
Rastrichness<-ggexport(Rastrichness, filename = "Rastrichness.pdf")
...
TtlPopAma <- ggplot(AmaTtlPopData, aes(x=patch, y=mean_n)) + 
  labs(title="Amalgamation Generated Landscape",x=" ", y = "Mean Total population")+
  theme(plot.title = element_text(size=7.25),axis.text=element_text(size=7),
...
TtlPopRast <- ggplot(RastTtlPopData, aes(x=patch, y=mean_n)) + 
  labs(title="Raster Generated Landscape",x=" ", y = "Mean Total population")+
  theme(plot.title = element_text(size=7.25),axis.text=element_text(size=7),
        axis.title=element_text(size=9))+
  geom_boxplot()

TotalPopPlot<-ggarrange(TtlPopAma+
                        ...
                        TtlPopRast + 
                          theme(axis.text.y = element_blank(),
                                ...
                                axis.title.y = element_blank()),
                        nrow = 1)
TotalPopPlot<-annotate_figure(TotalPopPlot, top = text_grob(("Plots of Mean Total Population per Percentage Cover Under Different Landscape Generation")))
ggexport(TotalPopPlot, filename = "TotalPopMulti.pdf")
#Species Richness 
...
SppRchAma <- ggplot(AmaSppRchData, aes(x=patch, y=mean_Rich)) + 
  labs(title="Amalgamation Generated Landscape",x=" ", y = "Species Richness")+
  theme(plot.title = element_text(size=7.25),axis.text=element_text(size=7),
        axis.title=element_text(size=9))+
  geom_boxplot()
SppRchRand <- ggplot(RandSppRchData, aes(x=patch, y=mean_Rich)) + 
  labs(title="Random Generated Landscape",x="Percentage Cover", y = "Species Richness")+
  theme(plot.title = element_text(size=7.25),axis.text=element_text(size=7),
        axis.title=element_text(size=9))+
  geom_boxplot()
SppRchRast <- ggplot(RastSppRchData, aes(x=patch, y=mean_Rich)) + 
  labs(title="Raster Generated Landscape",x=" ", y = "Species Richness")+
  theme(plot.title = element_text(size=7.25),axis.text=element_text(size=7),
        axis.title=element_text(size=9))+
  geom_boxplot()

SpeciesRichnessPlot<- ggarrange(SppRchAma+
                                  theme(axis.ticks.y = element_blank()),
                                SppRchRand + 
                                  theme(axis.text.y = element_blank(),
                                        axis.ticks.y = element_blank(),
                                ...
                                SppRchRast+ 
                                  theme(axis.text.y = element_blank(),
                                        ...
                                        axis.title.y = element_blank()), 
                                nrow = 1)
SpeciesRichnessPlot<-annotate_figure(SpeciesRichnessPlot, top = text_grob(("Plots of Mean Species Richness per Percentage Cover Under Different Landscape Generation")))
ggexport(SpeciesRichnessPlot, filename = "SpeciesRichMulti.pdf")
...
FullTtlPopLm<-summary(lm(Mean_n ~ Patch+LandGen, FullTtlpopData))
...
FullSppRchLm<-summary(lm(Mean_Rich ~ Patch+LandGen, FullRichData))

CombineTotalPop<-ggplot(FullTtlpopData, aes(x=Patch, y=Mean_n)) + 
  labs(title="Plot of Mean Total per Percentage Cover",x="Percentage Cover", y = "Mean Total population")+
  geom_boxplot()
ggexport(CombineTotalPop, filename = "CombineTotalPop.pdf")

CombineSpeciesRich<-ggplot(FullRichData, aes(x=Patch, y=Mean_Rich)) + 
  labs(title="Plot of Species Richness per Percentage Cover",x="Percentage Cover", y = "Species Richness")+
  geom_boxplot()
ggexport(CombineSpeciesRich, filename = "CombineSpeciesRich.pdf")
