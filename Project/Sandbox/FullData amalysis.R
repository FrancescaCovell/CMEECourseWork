...
gpath = "/home/frcovell/Project/sandbox/Sumplimentary/LandGeneration"
setwd(gpath)
...
RandtotalPops_SE = RandtotalPops %>% group_by(g, patch) %>% summarise(mean_n = mean(n), SE = SE(n)) 
RandtotalPops<-ggplot(RandtotalPops_SE, aes(x = g, y = mean_n, fill = patch)) + 
  geom_line() +
  geom_ribbon(aes(ymax = mean_n + SE, ymin = mean_n - SE), alpha = 0.2) + 
  labs(title ="Random Landscape Cover", x = "Generation", y = "Mean Total Population") + 
  theme_bw()
ggexport(RandtotalPops, filename = "RandtotalPops.pdf")
